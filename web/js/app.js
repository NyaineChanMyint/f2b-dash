...
(function () {
  ...
  var selectedHost = localStorage.getItem('f2b-selected-host') || '';

  async function loadHosts() {
    var select = document.getElementById('host-select');
    if (!select) return;
    try {
      var response = await fetch(CONFIG.hostsApi + '?t=' + Date.now());
      ...
      hosts.forEach(function (host) {
        ...
        option.value = host.name; option.textContent = host.name;
        if (host.name === selectedHost) option.selected = true;
        select.appendChild(option);
      });
      if (selectedHost && !hosts.some(function (host) { return host.name === selectedHost; })) {
        selectedHost = ''; localStorage.removeItem('f2b-selected-host');
      }
  ...
  /**
   * Render the last 7 days summary card.
   * Sums the last 7 daily trend entries (or fewer if unavailable) for
   * total attacks, bans, and unbans.
   * @param {object} data - Full dashboard JSON object
   */
  function renderWeeklyNew(data) {
    var attacksEl = document.getElementById('weekly-attacks');
    var bansEl = document.getElementById('weekly-bans');
    var unbansEl = document.getElementById('weekly-unbans');
    if (!attacksEl || !bansEl || !unbansEl) return;

    var trends = data && data.trends;
    if (!trends || !trends.length) {
      attacksEl.textContent = '--';
      bansEl.textContent = '--';
      unbansEl.textContent = '--';
      return;
    }

    var windowSize = Math.min(7, trends.length);
    var startIdx = trends.length - windowSize;
    var attacks = 0;
    var bans = 0;
    var unbans = 0;

    for (var i = startIdx; i < trends.length; i++) {
      var day = trends[i];
      attacks += day.total || 0;
      bans += day.bans || 0;
      unbans += day.unbans || 0;
    ...
    attacksEl.textContent = Number(attacks).toLocaleString();
    bansEl.textContent = Number(bans).toLocaleString();
    unbansEl.textContent = Number(unbans).toLocaleString();
  }
  ...
  function renderPerJailContent(jail, data) {
    ...
    var bannedIPs = jailData.bannedIPs || [];
    var bannedHTML = '<div class="jail-report-card">' +
      '<div class="jail-report-card__title"' + titleStyle + '>' + t('perJail.bannedIPs') + '</div>';

    if (bannedIPs.length) {
      bannedHTML += '<div class="ip-table-wrapper ip-table-wrapper--banned"><table class="ip-table">' +
        '<thead><tr>' +
          '<th>' + t('topIPs.ip') + '</th>' +
          '<th>' + t('topIPs.country') + '</th>' +
          '<th>' + t('perJail.bannedAt') + '</th>' +
        '</tr></thead><tbody>';

      for (var b = 0; b < bannedIPs.length; b++) {
        var bIP = bannedIPs[b];
        var bFlag = getCountryFlag(bIP.country);
        var bCountry = bFlag ? bFlag + ' ' + (bIP.country || '--') : (bIP.country || '--');
        bannedHTML += '<tr>' +
          '<td class="ip-table__ip">' + escapeHtml(bIP.ip || '--') + '</td>' +
          '<td class="ip-table__country">' + escapeHtml(bCountry) + '</td>' +
          '<td class="ip-table__last-seen">' + formatLogTimestamp(bIP.lastBannedAt) + '</td>' +
        '</tr>';
      }

      bannedHTML += '</tbody></table></div>';
    } else {
      bannedHTML += '<div class="empty-state">' + t('perJail.bannedEmpty') + '</div>';
    }
    bannedHTML += '</div>';
    ...
    container.innerHTML = toggleHTML + configHTML + statsHTML + ipsHTML + bannedHTML + chartHTML;
  ...
  async function fetchAndRender() {
    try {
      const url = CONFIG.dashboardApi + '?t=' + Date.now() + (selectedHost ? '&host=' + encodeURIComponent(selectedHost) : '');
      var response;
      try {
      ...
      if (data.trends && data.trends.length) {
        ...
        renderWeeklyNew(data);
      }

      if (data.topIPs && data.topIPs.length) {
        renderTopIPsTable(data.topIPs);
        renderWorldMap('chart-world-map', data.topIPs, data.server);
        renderRegionChart('chart-region', data.topIPs);
      }
  ...
  function onLanguageChanged() {
    if (currentData) {
      ...
      if (currentData.trends && currentData.trends.length) {
        renderTrendChart('chart-trend', currentData.trends, currentTimeRange);
        renderWeeklyNew(currentData);
      }
      if (currentData.topIPs) {
        renderTopIPsTable(currentData.topIPs);
        renderWorldMap('chart-world-map', currentData.topIPs, currentData.server);
        renderRegionChart('chart-region', currentData.topIPs);
      }
  ...
  async function init() {
    ...
    if (hostSelect) {
      hostSelect.addEventListener('change', function () {
        ...
        localStorage.setItem('f2b-selected-host', selectedHost);
        fetchAndRender();
      });
    }
    ...
    // Initial host list and data fetch
    await loadHosts();
